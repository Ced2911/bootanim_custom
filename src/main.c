...
// #define VDP2_MAP_ADDR(bank, offset) (((bank << 17) + offset) >> 12)
//#define VDP2_MAP_ADDR(offset) (offset >> 12)
...
static screen_settings_struct screen_0 = {
    .is_bitmap = 0,
    .color = BG_16COLOR,
    .pattern_name_size = sizeof(page_t) / 2,
    .char_size = 2,
    .special_priority = 0,
    .map = {
        (uint8_t)VDP2_MAP_ADDR(PLAN_A_OFFSET),
        (uint8_t)VDP2_MAP_ADDR(PLAN_A_OFFSET),
        (uint8_t)VDP2_MAP_ADDR(PLAN_A_OFFSET),
        (uint8_t)VDP2_MAP_ADDR(PLAN_A_OFFSET)}};

static screen_settings_struct screen_1 = {
    .is_bitmap = 0,
    .color = BG_16COLOR,
    .pattern_name_size = sizeof(page_t) / 2,
    .char_size = 2,
    .special_priority = 0,
    .map = {
        (uint8_t)VDP2_MAP_ADDR(PLAN_B_OFFSET),
        (uint8_t)VDP2_MAP_ADDR(PLAN_B_OFFSET),
        (uint8_t)VDP2_MAP_ADDR(PLAN_B_OFFSET),
        (uint8_t)VDP2_MAP_ADDR(PLAN_B_OFFSET)}};
...
static void _setTileMap(const uint8_t *tilemap, page_t *pages)
{
    ...
    // const page_t pnd = VDP2_SCRN_PND_CONFIG_0(1, cpd_adr, pal_adr, y_flip, x_flip);
...
{
    // Init vdp
    vdp_nbg0_init(&screen_0);
    vdp_nbg1_init(&screen_1);
    ...
    vdp_set_priority(SCREEN_NBG0, 3);
